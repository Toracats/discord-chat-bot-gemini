...
def main(page: ft.Page):
    ...
    def load_initial_settings():
        ...
        try:
            token_field.value = config_manager.get_discord_token() or ""; gemini_api_key_field.value = config_manager.get_gemini_api_key() or ""
            weather_api_key_field.value = config_manager.get_weather_api_key() or ""; delete_password_field.value = config_manager.get_delete_history_password() or ""
            weather_auto_update_switch.value = config_manager.get_weather_auto_update_enabled(); weather_interval_field.value = str(config_manager.get_weather_auto_update_interval())
            logger.info("Initial settings loaded.")
        except Exception as e_load: logger.error("Error loading initial settings", exc_info=e_load); status_snackbar.content = ft.Text(f"設定読込エラー: {e_load}"); status_snackbar.open = True
    ...
    def on_window_event(e):
         if e.data == "close":
             ...
             logger.info("Signaling log forwarder stop..."); stop_log_forwarding()
